/*
 * Bootstrap Extension v5.1.0 (https://bootstrap-extension.com/)
 * Copyright 2018-2022 The Bootstrap Extension Author: Peter Joiner
 * Copyright 2018-2022 A.K.K., Inc.
 * Licensed under MIT
 */
...
docReady(function () {
    ...
    set_multi_navbar = () => {
        ...
        if (window.scrollY > 150 && multi_navbar_topbar && multi_navbar_topbar_Collapsable) {
            multi_navbar_topbar.style.height = '0px';
        ...
        if (window.scrollY < 50 && multi_navbar_topbar) {
            multi_navbar_topbar.style.height = '24px';
    ...
    if (multi_navbar) {
        ...
        document.querySelectorAll('.multi-navbar nav ul li a').forEach(function (menuItem, i) {
            menuItem.addEventListener('click', (event) => {
                multi_navbar_Nav_Ul.style.right = '-300px';
                if (i > 0 && menuItem.getAttribute('href').substring(0, 1) === '#') {
                    event.preventDefault();
                    var posY = document.querySelector(menuItem.getAttribute('href')).offsetTop - parseFloat(getComputedStyle(multi_navbar).getPropertyValue('height'));
                    window.scrollTo({
                        top: posY,
                        behavior: 'smooth'
                    });
        ...
        document.querySelector('.btn-hamburger').addEventListener('click', () => {
            multi_navbar_Nav_Ul.style.right = '0';
        });
    }
    ...
    function counterPosListener() {
        for (var i = 0; i < counters.length; i++) {
            if (countersTimers[i] === null && parseInt(window.scrollY) + parseInt(window.innerHeight) > getOffset(counters[i]).top) {
                ...
                countersTimers[i] = setInterval(function () {
                    if (parseInt((targetValue / (COUNTERTIMESEC * 1000 / 5)) * aktcTi) > targetValue) {
                        clearInterval(countersTimers[i]);
                    } else {
                        aktCounter.innerHTML = parseInt((targetValue / (COUNTERTIMESEC * 1000 / 5)) * aktcTi);
                    }
                    aktcTi++;
                }
                , COUNTERTIMESEC);
